...
def API_RunTime_DownTime():
    ...
    cur1 = conn.cursor()
    cur1.execute(" SELECT TOP(1) iov.MachineID , ppt.PlannedCode ,ppt.  from OEE_DB.dbo.INF_OEE1_V2 iov INNER JOIN OEE_DB.dbo.PlannedProductionTime ppt ON iov.MachineID = ppt.MachineID AND iov.PDOrder = ? AND ppt.[Date] = ? order by ppt.[DateTime] DESC ",(data['PDOrder'] ,data['RunTime'][0]['PostDate']))
