...
def Report_OEE_API():
    ...
    for result in ReportOEE:
        content = {'Plant': result[2], 'work_time': str(result[34] +" / " + result[33] ), 'Posting_Date': str(result[3]),'PD_order': result[4],'Material_number': str(result[5]),'Machine_Text': str(result[8]),'Material_Description': str(result[6]),'MachineID': str(result[7]),'Validate_Speed': str(result[9]),'Q1':  float("{:.2f}".format(result[25] * 100 )),'Plan_DT': str(result[10]),'Unlan_DT': str(result[11]),'ka_time': str(result[12]),'getwork_time_1': str(result[13]),'getwork_time_2': str(result[14]),'number_of_product': str(result[15]),'number_should_of_product_1':  float("{:.2f}".format(result[16])),'number_should_of_product_2':  float("{:.2f}".format(result[17])),'product_Qty': str(result[18]),'Return_Qty': str(result[19]),'product_Qty_F': str(result[20]),'UserGroup': str(result[32]),'Q2': float("{:.2f}".format(result[38] *100)),'Availability_A1': float("{:.2f}".format(result[21]*100)),'Availability_A2': float("{:.2f}".format(result[22]*100)),'Performance_P1': float("{:.2f}".format(result[23]*100)),'Performance_P2': float("{:.2f}".format(result[24]*100)),'Quality1F': float("{:.2f}".format(result[26]*100)),'Quality2F': float("{:.2f}".format(result[39]*100)),'OEE1': float("{:.2f}".format(result[27]*100)),'OEE2': float("{:.2f}".format(result[28]*100)),'OEE1_F': float("{:.2f}".format(result[29]*100)),'OEE2_F': float("{:.2f}".format(result[30]*100))}
        payload.append(content)
        content = {}
    #print(payload)
    return json.dumps({"data":payload}, cls = Encoder), 201
