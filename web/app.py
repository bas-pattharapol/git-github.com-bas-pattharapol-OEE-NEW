    ...
    for p in range(0,len(data['Result'])):
        for i in range(0,len(data['Result'][p]['Machine'])):
            ...
            for j in range(0,len(data['Result'][p]['Machine'][i]['GR_QTY'])):
                ...
                cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
                update = cnxn.cursor()
                update.execute('INSERT INTO OEE_DB.dbo.INF_OEE3_V2 (PDOrder,MachineID, QTY, [Date], [Time], [Date]) VALUES(?,?,?,?,?,?)' ,(data['Result'][p]['PDOrder'],data['Result'][p]['Machine'][i]['ID'],data['Result'][p]['Machine'][i]['GR_QTY'][j]['QTY'],data['Result'][p]['Machine'][i]['GR_QTY'][j]['Date'],data['Result'][p]['Machine'][i]['GR_QTY'][j]['Time']))
                cnxn.commit()
            print("------------------------------------")
