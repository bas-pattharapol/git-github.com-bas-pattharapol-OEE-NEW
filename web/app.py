...
def Edit_StorageTanks(mode,id,Level,Fname_Lname):
    ...
    if request.method == 'POST':
        ...
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        update = cnxn.cursor()
        update.execute('INSERT INTO OEE_DB.dbo.EditRecord (EditID,ChangeTopic,NameValue,[User],UserLevel) VALUES(?,?,?,?,?)' ,(1,mode,StorageName,User,UserLevel))
        cnxn.commit()   
...
def Edit_Machines(mode,id,Level,Fname_Lname):
    if request.method == 'POST':
        ...
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        update = cnxn.cursor()
        update.execute('INSERT INTO OEE_DB.dbo.EditRecord (EditID,ChangeTopic,NameValue,[User],UserLevel) VALUES(?,?,?,?,?)' ,(2,mode,MachineName,User,UserLevel))
        cnxn.commit()  
...
def Edit_Plant(mode,id,Level,Fname_Lname):
    if request.method == 'POST':
        ...
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        update = cnxn.cursor()
        update.execute('INSERT INTO OEE_DB.dbo.EditRecord (EditID,ChangeTopic,NameValue,[User],UserLevel) VALUES(?,?,?,?,?)' ,(3,mode,PlantName,User,UserLevel))
        cnxn.commit() 
...
def Edit_DownTimeCode(mode,id,Level,Fname_Lname):
    if request.method == 'POST':
        ...
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        update = cnxn.cursor()
        update.execute('INSERT INTO OEE_DB.dbo.EditRecord (EditID,ChangeTopic,NameValue,[User],UserLevel) VALUES(?,?,?,?,?)' ,(4,mode,Description,User,UserLevel))
        cnxn.commit()      
    if mode == "del":
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        DownTimeCode = cnxn.cursor()
...
def Edit_Shift(mode,id,Level,Fname_Lname):
    if request.method == 'POST':
        ...
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        update = cnxn.cursor()
        update.execute('INSERT INTO OEE_DB.dbo.EditRecord (EditID,ChangeTopic,NameValue,[User],UserLevel) VALUES(?,?,?,?,?)' ,(5,mode,ShiftCodeName,User,UserLevel))
        cnxn.commit() 
...
def Edit_Standardcolor(mode,id,Level,Fname_Lname):
    if request.method == 'POST':
        ...
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        update = cnxn.cursor()
        update.execute('INSERT INTO OEE_DB.dbo.EditRecord (EditID,ChangeTopic,NameValue,[User],UserLevel) VALUES(?,?,?,?,?)' ,(6,mode,Revision,User,UserLevel))
        cnxn.commit()     
...
def Edit_UserGroup(mode,id,Level,Fname_Lname):
    if request.method == 'POST':
        ...
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        update = cnxn.cursor()
        update.execute('INSERT INTO OEE_DB.dbo.EditRecord (EditID,ChangeTopic,NameValue,[User],UserLevel) VALUES(?,?,?,?,?)' ,(7,mode,UserGroupName,User,UserLevel))
        cnxn.commit()
...
def Edit_Department(mode,id,Level,Fname_Lname):
    if request.method == 'POST':
        ...
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        update = cnxn.cursor()
        update.execute('INSERT INTO OEE_DB.dbo.EditRecord (EditID,ChangeTopic,NameValue,[User],UserLevel) VALUES(?,?,?,?,?)' ,(8,mode,Department_Name,User,UserLevel))
        cnxn.commit()      
    if mode == "del":
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password)
        Department = cnxn.cursor()
...
def Report_OEE_API():
    ...
    for result in ReportOEE:
        content = {'Plant': result[2], 'work_time': str(result[34] +" / " + result[33] ), 'Posting_Date': str(result[3]),'PD_order': result[4],'Material_number': str(result[5]),'Machine_Text': str(result[8]),'Material_Description': str(result[6]),'MachineID': str(result[7]),'Validate_Speed': str(result[10]),'Q1':  float("{:.2f}".format(result[25])),'Plan_DT': str(result[10]),'Unlan_DT': str(result[11]),'ka_time': str(result[12]),'getwork_time_1': str(result[13]),'getwork_time_2': str(result[14]),'number_of_product': str(result[15]),'number_should_of_product_1':  float("{:.2f}".format(result[16])),'number_should_of_product_2':  float("{:.2f}".format(result[17])),'product_Qty': str(result[18]),'product_Qty': str(result[18]),'Return_Qty': str(result[19]),'product_Qty_F': str(result[20]),'UserGroup': str(result[32]),'Q2': float("{:.2f}".format(result[38])),'Availability_A1': float("{:.2f}".format(result[21])),'Availability_A2': float("{:.2f}".format(result[22])),'Performance_P1': float("{:.2f}".format(result[23])),'Performance_P2': float("{:.2f}".format(result[24])),'Quality1F': float("{:.2f}".format(result[26])),'Quality2F': float("{:.2f}".format(result[38])),'OEE1': float("{:.2f}".format(result[27])),'OEE2': float("{:.2f}".format(result[28])),'OEE1_F': float("{:.2f}".format(result[29])),'OEE2_F': float("{:.2f}".format(result[30]))}
        payload.append(content)
        content = {}
    #print(payload)
    return json.dumps({"data":payload}, cls = Encoder), 201
...
def Report_OEE_API_EXCEL():
    ...
    content = {}
    for result in ReportOEE:
        content = {'Plant': result[2], 'กะการทำงาน': str(result[34] +" / " + result[33] ), 'Posting Date': str(result[3]),'PD order': result[4],'Material number': str(result[5]),'Machine Text': str(result[8]),'Material Description': str(result[6]),'MachineID': str(result[7]),'Validate Speed': str(result[10]),'Plan DT': str(result[10]),'Unlan DT': str(result[11]),'กะเวลา (min)': result[12],'เวลารับภาระงาน1 (min)': str(result[13]),'เวลารับภาระงาน2 (min)': str(result[14]),'จำนวนชิ้นงานที่ผลิตได้ (Unit)': str(result[15]),'จำนวนชิ้นงานที่ควรจะผลิตได้1 (Unit)':  float("{:.2f}".format(result[16])),'จำนวนชิ้นงานที่ควรจะผลิตได้2 (Unit)':  float("{:.2f}".format(result[17])),'จำนวนชิ้นงานที่ผ่านคุณภาพ (Unit)': str(result[18]),'product_Qty': str(result[18]),'Return Qty (after 30 day) (Unit)': str(result[19]),'จำนวนชิ้นงานที่ผ่านคุณภาพสุดท้าย (Unit)': str(result[20]),'UserGroup': str(result[32]),'Availability (A1)': float("{:.2f}".format(result[21])),'Availability (A2)': float("{:.2f}".format(result[22])),'Performance (P1)': float("{:.2f}".format(result[23])),'Performance (P2)': float("{:.2f}".format(result[24])),'Quality by PD order (Q1)':  float("{:.2f}".format(result[25])),'Quality by shift (Q2)': float("{:.2f}".format(result[38])),'Quality Final by PD order (Q1)': float("{:.2f}".format(result[26])),'Quality Final by Shift (Q2)': float("{:.2f}".format(result[38])),'OEE1': float("{:.2f}".format(result[27])),'OEE2': float("{:.2f}".format(result[28])),'OEE1_F': float("{:.2f}".format(result[29])),'OEE2_F': float("{:.2f}".format(result[30]))}
        payload.append(content)
        content = {}
    #print(payload)
