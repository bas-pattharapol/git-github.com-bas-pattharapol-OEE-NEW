...
def Report_M_OEE_API():
    ...
    for result in OEEReport_Monthly_Show:
        ...
        content = {'Month': result[1] , 'Plant': result[3],'MachineID': str(result[4]),'Machine_Text': str(result[5]),'PlanDownTime':  result[6],'UnplanDownTime': str(result[7]),'RunTime1': str(result[8]),'RunTime2': str(result[9]),'Per_PantDownTime':  str(float("{:.2f}".format(result[10])) * 100) + '%','Per_UnplanDowntime':  str(float("{:.2f}".format(result[11])) * 100) + '%','Per_Downtime': str(float("{:.2f}".format(result[12])) * 100) + '%','Per_PantDownTime2': str(float("{:.2f}".format(result[13])) * 100) + '%','Per_UnplanDowntime2': str(float("{:.2f}".format(result[14])) * 100) + '%','Per_Downtime2':str(float("{:.2f}".format(result[15])) * 100) + '%','TotalCount': float("{:.2f}".format(result[16])),'IdealCount1': float("{:.2f}".format(result[17])),'IdealCount2': float("{:.2f}".format(result[18])),'GoodCount': float("{:.2f}".format(result[19])),'PostReturn': float("{:.2f}".format(result[20])),'FinalGoodCount': float("{:.2f}".format(result[21])),'OEE_A1': str(float("{:.2f}".format(result[22])) * 100) + '%','OEE_A2': str(float("{:.2f}".format(result[23])) * 100) + '%','OEE_P1': str(float("{:.2f}".format(result[24])) * 100) + '%','OEE_P2': str(float("{:.2f}".format(result[25])) * 100) + '%','OEE_Q': str(float("{:.2f}".format(result[26])) * 100) + '%','OEE_Q_Finnal': str(float("{:.2f}".format(result[27])) * 100) + '%','OEE1Calculation': str(float("{:.1f}".format(result[28])) * 100) + '%','OEE2Calculation': str(float("{:.2f}".format(result[29])) * 100) + '%','OEE1FinalCalculation': str(float("{:.2f}".format(result[30])) * 100) + '%','OEE2FinalCalculation': str(float("{:.2f}".format(result[31])) * 100) + '%'}
        payload.append(content)
        content = {}
