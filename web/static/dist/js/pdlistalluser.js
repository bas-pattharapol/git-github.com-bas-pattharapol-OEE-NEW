...
/************* Send To PLC **************/
	$(document).delegate('.btnsynctoplc', 'click', function() {
		var controlId = $(this).attr('id');
		var actionKey = controlId.substr(controlId.indexOf('_')+1);
		// var useraccid = $('#lbl_orderlist_'+actionKey).html();
		$('#cur_spw_key').val(actionKey);
		$('#modal_dialog').removeClass('modal-success').addClass('modal-warning');
		$('.modal-header').children('h4').html('Sync Mixing User to PLC confirmation!');
		$('.modal-body').children().html('Are you sure you want to Sync Mixing User to PLC ?');
		$('.modal-footer').children('.btn:nth-child(1)').css('display','');
		$('.modal-footer').children('.btn:nth-child(1)').attr('id','btnCfSyncPLC');
		$('.modal-footer').children('.btn:nth-child(1)').html('Clear');
		$('.modal-footer').children('.btn:nth-child(2)').css('display','');
		$('.modal-footer').children('.btn:nth-child(2)').attr('id','btnCfDonotSyncPLC');
		$('.modal-footer').children('.btn:nth-child(2)').html('Cancel');
		$('#modal_dialog').modal('show');
	});

	$(document).delegate('#btnCfDonotSyncPLC', 'click', function() {
	...
	$(document).delegate('#btnCfSyncPLC', 'click', function() {
