...
/************************************* SEND TO PREWEIGHT *************************************/
	...
	$(document).delegate('#btnCfSPW', 'click', function() {
		...
		acq_sendtopreweight($('#cur_spw_key').val(), $('#cur_spw_batchnokey').val(), $('#cur_versionproduct').val());
...
/************************************* SEND TO STATION *************************************/
	$('.btnsendstation').on('click', function() {
		var controlId = $(this).attr('id');
		var actionKey = controlId.substr(controlId.indexOf('_')+1);
	   console.log('xxx' + actionKey);	
		var pd_rmid = $('#lbl_pdorder_'+actionKey).html();
		// var pd_rmid = $('#lbl_pdorder_'+actionKey).val();
		
		// var pd_station = $('#pwstation_'+actionKey).html();
		var pd_station = $('#pwstation_'+actionKey+' option:selected').val();   
	   var pd_fullfilltank = $('#fulltanksize_'+actionKey).val();
		var pd_displayconfirm = $('#lbl_confirm_'+actionKey).html();
		
		
		// var result_arr = pd_batchno.split('-');

		 console.log('pd_rmid ' + pd_rmid);
		 console.log('pd_station ' + pd_station);
		console.log('pd_fullfilltank ' + pd_fullfilltank);
		
		$('#cur_spw_key').val(pd_rmid);	
		$('#cur_spw_station').val(pd_station);
		$('#cur_spw_fulltanksize').val(pd_fullfilltank);		
		
		$('#modal_dialog').removeClass('modal-success').addClass('modal-warning');
		$('.modal-header').children('h4').html('Send Pre-Weight Station confirmation!');
		$('.modal-body').children().html('Are you sure you want to Send '+pd_displayconfirm+' To Pre-Weight Station"?');
		$('.modal-footer').children('.btn:nth-child(1)').css('display','');
		$('.modal-footer').children('.btn:nth-child(1)').attr('id','btnCfSentSTT');
		$('.modal-footer').children('.btn:nth-child(1)').html(' Set ');
		$('.modal-footer').children('.btn:nth-child(2)').css('display','');
		$('.modal-footer').children('.btn:nth-child(2)').attr('id','btnCfDonotSPW');
		$('.modal-footer').children('.btn:nth-child(2)').html('Cancel');
		$('#modal_dialog').modal('show');
	...
	$(document).delegate('#btnCfSentSTT', 'click', function() {

		acq_sendtopreweightstt($('#cur_spw_key').val(), $('#cur_spw_station').val(), $('#cur_spw_fulltanksize').val() );
...
/************************************* Revoke *************************************/
	$('.btnrevoke').on('click', function() {
		...
		var pd_batchno = $('#lbl_batchno_'+actionKey).html();
		var pd_version = $('#lbl_pdversion_'+actionKey).html();
		var pd_display_name = $('#lbl_confirm_'+actionKey).html();
		
		console.log('xxx' + actionKey);
		console.log('pd_order ' + "lbl_pdorder_"+actionKey);
		console.log('pd_version ' + pd_version);
		console.log('pd_display_name ' + pd_display_name);
		
		var rawid = pd_display_name.split('/',2)[0];
		var rawname = pd_display_name.split('/',2)[1];
		
		console.log('rawid, rawname ' + rawid+","+rawname);
				
		$('#cur_spw_key').val(actionKey);
		$('#cur_spw_displayname').val(pd_display_name);
		$('#cur_spw_rawidsel').val(rawid);
		$('#cur_spw_rawnamesel').val(rawname);
		$('#modal_dialog').removeClass('modal-success').addClass('modal-warning');
		$('.modal-header').children('h4').html('Revoke RawMaterial confirmation!');
		$('.modal-body').children().html('Are you sure you want to Revoke RawMaterial \"'+pd_display_name+'\"?');
		$('.modal-footer').children('.btn:nth-child(1)').css('display','');
		$('.modal-footer').children('.btn:nth-child(1)').attr('id','btnCfREV');
		$('.modal-footer').children('.btn:nth-child(1)').html('Send');
		$('.modal-footer').children('.btn:nth-child(2)').css('display','');
		$('.modal-footer').children('.btn:nth-child(2)').attr('id','btnCfDonotREV');
		$('.modal-footer').children('.btn:nth-child(2)').html('Cancel');
		$('#modal_dialog').modal('show');
	});

	$(document).delegate('#btnCfDonotREV', 'click', function() {
	...
	$(document).delegate('#btnCfREV', 'click', function() {
  
		acq_sendtorevoke($('#cur_spw_key').val(), $('#cur_spw_rawidsel').val(), $('#cur_spw_rawnamesel').val());
...
function acq_sendtopreweightstt(pd_rmid, pd_station, pd_fullfilltank ){

	 alert('pd_rmid = '+pd_rmid+' pd_station '+pd_station+' pd_fullfilltank '+pd_fullfilltank);
	$.ajax({
	  type: "POST",
	  url: "ajax_rm_set_station.php",
	  data: {
	  	"pd_rmid": pd_rmid,
	  	"pd_station": pd_station,
	  	"pd_fullfilltank":pd_fullfilltank
//	  	"acq_desc": acq_desc
		},
	  cache: false,
	  beforeSend: function() {
...
function acq_sendtorevoke(pd_order, rawid, rawname) {
...
// async:true = Code continued. (Nothing gets paused. Other code is not waiting.)
	...
	$.ajax({
	  type: "POST",
	  url: "ajax_rawmaterial_revoke.php",
	  async:false,  
	  data: {
	  	"pd_order": pd_order,
	  	"rawid": rawid,
